...
test('normalizes a Gemma endpoint advisory without granting it decision authority', async () => {
  const client = { predict: async ({ endpoint, instances }) => {
    assert.equal(endpoint, 'projects/test-project/locations/us-central1/endpoints/123');
    assert.equal(instances.length, 1);
    return [{ predictions: [{ stringValue: JSON.stringify({ risk: 'flagged', instructionLikeText: true, unsupportedClaim: false, shouldAbstain: false, flags: ['instruction_like_source_text'] }) }] }];
  } };
