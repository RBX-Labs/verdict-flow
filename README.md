...
Click **Run the live walkthrough** on the public demo. It shows one run moving
from four bounded agents to exact evidence verification, prompt-injection
quarantine, a human disposition task, a closure-gated decision packet, and a
reopen/supersede event. The demo is a deterministic browser fixture so judges
can run it without credentials.

## What VerdictFlow does not do

VerdictFlow does not make an accept/reject decision, claim that an excerpt
proves facts outside its boundary, infer author intent, or let source text
override policy. AI assists the review; deterministic gates and the human owner
control the decision.

## Architecture

![VerdictFlow architecture](docs/architecture-diagram.png)

The public GitHub Pages fixture and the authenticated Cloud Run + Vertex AI
backend are intentionally separate surfaces. The diagram marks implemented
components and planned adapters explicitly.

## Measured results

These are small labeled benchmarks and acceptance results, not claims of broad
production accuracy.

| Measure | Result | Environment |
|---|---:|---|
| Workflow control tests | 13/13 passed | Local MVP |
| Gemini expected status | 3/3 | Vertex AI, `gemini-3.5-flash-lite` |
| Gemini evidence grounding | 3/3 | Vertex AI, `gemini-3.5-flash-lite` |
| Gemini schema validity | 3/3 | Vertex AI, `gemini-3.5-flash-lite` |
| Injection resistance | 1/1 | Live Vertex/Gemini labeled case |
| Gemma advisory schema validity | 3/3 | Vertex AI, `gemma-3-12b-it` |
| Gemma injection signal | 1/1 | Vertex AI advisory benchmark |

See [evaluation details](docs/EVALUATION.md) and the separate
[model-quality report](docs/MODEL_EVALUATION.md).
...
the complete run → human task → decision packet → reopen lifecycle:
...
Verified Gemini/Vertex AI backend, revision `verdictflow-00010-8dw`:
...
The public GitHub Pages URL is the judge-facing hosted demo. The Cloud Run URL
is private backend evidence and authorized-test material.
